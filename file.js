...
const TIERS = [
    ...
    { name: 'Rare', level: 3, color: '#4bf', mult: 1.6, mods: 3 },
    { name: 'Epic', level: 4, color: '#aa33ff', mult: 2.0, mods: 4 },
    { name: 'Legendary', level: 5, color: '#f82', mult: 2.8, mods: 5 }
];

const ITEM_TEMPLATES = {
    'Primary Weapon': { names: ['Pulse Laser', 'Plasma Repeater', 'Ion Cannon'], stats: ['damage', 'critChance'] },
...
function generateLoot(type = null, tierLevel = -1) {
    ...
    if (Math.random() < 0.25 && type === null) {
        ...
        return {
            ...
            tier: 0,
            stackable: true,
            count: isFuel ? 12 : MathUtils.randInt(1, 5),
            desc: isFuel ? 'Restores 12 Fuel on use.' : 'Can be traded or scrapped.'
        };
    }
    ...
    if (tierLevel === -1) {
        const r = Math.random();
        if (r > 0.995) tierLevel = 5; 
        else if (r > 0.97) tierLevel = 4; 
        else if (r > 0.90) tierLevel = 3; 
        else if (r > 0.60) tierLevel = 2; 
        else tierLevel = 1; 
    }
    
    if (player.level === 1 && tierLevel === 2 && Math.random() > 0.2) {
        tierLevel = 1; // Reduce uncommon chance at level 1
    }
    ...
    for (let stat of pickedStats) {
        let val = 0; let str = '';
        if (stat === 'fireRateRating') { val = Math.floor(MathUtils.rand(5, 15) * tier.mult); str = `+${val} Fire Rate Rating`; }
        ...
        else if (stat === 'armor') { val = Math.floor(MathUtils.rand(2, 10) * tier.mult * lvlMult); str = `+${val} Armor`; }
        ...
        if (val > 0) { stats[stat] = val; statLines.push(str); }
    }
...
function getFireRateBonus(rating, level) {
    // Converts flat rating into a percentage bonus, with diminishing returns vs level.
    return (rating / (rating + 40 + level * 8));
}
...
const BASE_STATS = {
    ...
    maxFuel: 100, fuel: 100, fuelRegen: 1 / 3,
...
const player = {
    ...
    gainXp(amount) {
        ...
        while (this.xp >= this.xpNext) {
            ...
            this.skills[0].cost = Math.floor(2 * (1 + 0.2 * (this.level - 1)));
    ...
    updateStats() {
        ...
        let fireRateBonus = getFireRateBonus(totalFireRateRating, this.level);
        ...
        if(this.hp <= 0) {
            if (source === player && equipment['Secondary Weapon'] && equipment['Secondary Weapon'].perk === 'Explosive Enemies') {
                ...
                for(let other of entities) {
                    if (other instanceof Enemy && other !== this && MathUtils.distance(this.x, this.y, other.x, other.y) < 400) {
                        other.takeDamage(player.stats.damage, player); 
                    }
                }
            }
            ...
            if(Math.random() < 0.3) {
                ...
                for(let i=0; i<numOrbs; i++) xpOrbs.push(new XpOrb(this.x, this.y, (player.level * 1) / numOrbs));
            }
            return true; // remove
        }
        return false;
...
class Enemy {
    ...
    update(dt) {
        ...
        let speedDebuff = this.disruptionTimer > 0 ? 0.05 : 1.0;
        ...
        if (this.type === 'chaser') {
            ...
            if(dist > 300) {
                this.vx = Math.cos(angle) * this.speed * speedDebuff;
                this.vy = Math.sin(angle) * this.speed * speedDebuff;
            } else if (dist < 200) {
                this.vx = -Math.cos(angle) * this.speed * speedDebuff;
                this.vy = -Math.sin(angle) * this.speed * speedDebuff;
            } else {
            ...
            if (this.attackTimer <= 0 && dist < 400 && this.disruptionTimer <= 0) {
                // Shoot player
                projectiles.push(new Projectile(this.x, this.y, angle, 300, this.damage, false, this.color, this));
    ...
    draw(ctx) {
        ...
        if (this.type === 'chaser' && this.attackCombo > 0) {
            ...
            else if (this.attackCombo === 3) comboColor = (Math.sin(Date.now()/100) > 0) ? 'rgba(255,128,0,0.7)' : 'rgba(255,255,0,0.7)';
    ...
    takeDamage(amount, source) {
        ...
        if(this.hp <= 0) {
            ...
            let xpPerOrb = (5 * this.level) / numOrbs;
...
class Projectile {
    constructor(x, y, angle, speed, damage, isPlayer, color, source) {
        this.x = x; this.y = y; this.z = 0;
            ...
            for(let i=entities.length-1; i>=0; i--) {
                ...
                if(MathUtils.distance(this.x, this.y, e.x, e.y) < e.radius) {
                    if (typeof e.takeDamage === 'function') {
                        if(e.takeDamage(this.damage, this.source)) entities.splice(i, 1);
        ...
        if (this.state === 'moving') {
            ...
            for(let e of entities) {
                ...
                if (d < this.radius && e.z <= 0) {
                    ...
                    if(e instanceof Enemy) e.takeDamage(player.stats.damage * dt, player); // dot damage scaling with player
                }
            }
            ...
            if (this.timer <= 0) {
                ...
                for(let i=entities.length-1; i>=0; i--) {
                    ...
                    if (MathUtils.distance(this.x, this.y, e.x, e.y) < this.radius) {
                        if (typeof e.takeDamage === 'function') {
                            if(e.takeDamage(player.stats.damage * 20, player)) entities.splice(i, 1);
                        }
                    }
                }
...
class DisruptionDebris {
    ...
    update(dt) {
        this.life -= dt;
        for(let e of entities) {
            if (e instanceof Enemy && MathUtils.distance(this.x, this.y, e.x, e.y) < this.radius + e.radius) {
                e.disruptionTimer = 1.0;
            }
        }
        return this.life <= 0;
    }
    draw(ctx) {
        let p = project(this.x, this.y, this.z);
        if(!p) return;
        ctx.fillStyle = `rgba(153, 51, 255, ${0.4 * (this.life/3.0)})`;
        ctx.beginPath(); ctx.arc(p.x, p.y, this.radius * getScale(this.z), 0, Math.PI*2); ctx.fill();
    }
}
...
function updateUI() {
    ...
    let fuelCellItem = inventory.find(i => i && i.type === 'Fuel');
    let fuelCellCount = fuelCellItem ? fuelCellItem.count : 0;
    document.getElementById('fuel-cell-count').innerText = fuelCellCount;

    let btnF = document.getElementById('skill-5');
    if (!fuelCellItem) btnF.classList.add('no-energy');
...
function showItemTooltip(item, e) {
    ...
    if (item.stats) {
        for (let stat in item.stats) {
            ...
            let str = item.statLines.find(l => l.includes(val) && !l.startsWith('[PERK]'));
            if (!str) str = `+${val} ${stat}`;
            
            if (delta > 0) str += ` <span style="color:#0f0">(+${delta})</span>`;
            else if (delta < 0) str += ` <span style="color:#f00">(${delta})</span>`;
            statsHtml.push(str);
        }
    }
    if (item.perk) {
    ...
    if (eqItem && item !== eqItem) {
        ...
        document.getElementById('tt-eq-type').innerText = eqItem.type;
...
function useSkill(index) {
    ...
    if(index === 0) { // Pulse Blaster
        ...
        if (hasTriple) {
            projectiles.push(new Projectile(player.x, player.y, angle, 600, player.stats.damage, true, varColor('--accent'), player));
            projectiles.push(new Projectile(player.x, player.y, angle - Math.PI/8, 600, player.stats.damage, true, varColor('--accent'), player));
            projectiles.push(new Projectile(player.x, player.y, angle + Math.PI/8, 600, player.stats.damage, true, varColor('--accent'), player));
        } else {
            projectiles.push(new Projectile(player.x, player.y, angle, 600, player.stats.damage, true, varColor('--accent'), player));
        }
        ...
        for(let i=1; i<=steps; i++) {
            disruptionDebris.push(new DisruptionDebris(MathUtils.lerp(oldX, player.x, i/steps), MathUtils.lerp(oldY, player.y, i/steps)));
        }
    }
...
function update(dt) {
    ...
    player.stats.fuel = Math.min(player.stats.maxFuel, player.stats.fuel + player.stats.fuelRegen * dt);
    ...
    for(let i=disruptionDebris.length-1; i>=0; i--) {
        if(disruptionDebris[i].update(dt)) disruptionDebris.splice(i, 1);
    }
...
function draw() {
    ...
    let allRenderables = [
        ...entities, ...drops, ...xpOrbs, ...particles, ...projectiles, ...disruptionDebris,
